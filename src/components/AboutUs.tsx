...
export const AboutUs: React.FC = () => {
  ...
  const kitchenScenes: KitchenScene[] = [
    {
      id: 'scene-sizzle',
      title: language === 'am'
        ? 'የዋናው ሼፍ ወጥ ቤት፡ ትኩስ የምግብ አበሳሰል'
        : "Chef's Kitchen: Sizzling Pan & Searing Station",
      subtitle: language === 'am' ? 'በየቀኑ ትኩስ የምግብ አሰራር' : 'Fresh culinary craft in every service',
      tag: language === 'am' ? 'ትኩስ ዝግጅት' : 'Kitchen Live',
      duration: '0:38',
    ...
    },
    {
      ...
      title: language === 'am'
        ? 'የተመረጡ የጾም ምግቦች ዝግጅት'
        : 'Curated Fasting Mastery & Seasonal Indulgence',
      subtitle: language === 'am' ? 'ጣፋጭ እና 100% የጾም አሰራር' : 'Flavorful dairy-free & plant-based culinary creations',
      tag: language === 'am' ? 'የጾም ወጥ ቤት' : 'Fasting Kitchen',
      duration: '0:42',
      description: language === 'am'
    ...
    },
    {
      id: 'scene-ambiance',
      title: language === 'am'
        ? 'የላ ቪ ኤን ሮዝ አየርና የግሪክ ደሴት ውበት'
        : 'Blooming Greek Island Atmosphere & Cafe Ambiance',
      subtitle: language === 'am' ? 'የፈረንሳይ እና የግሪክ ውብ ጌጣጌጥ' : 'Sunlit marble tables, floral rose arches & aesthetic vibes',
      tag: language === 'am' ? 'የካፌው ውበት' : 'Cafe Ambiance',
      duration: '0:35',
      description: language === 'am'
  ...
  const handleVideoEnded = () => {
    const nextIdx = (activeSceneIndex + 1) % kitchenScenes.length;
    ...
    <section id="about" className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-bold text-natural-heading leading-tight mb-6">
            {t('about.title')}
          </h2>
          <div className="relative p-6 sm:p-8 rounded-3xl bg-natural-bg border border-natural-border shadow-xs max-w-2xl mx-auto">
            <p className="font-serif text-lg sm:text-xl md:text-2xl text-natural-text italic leading-relaxed font-normal">
              {t('about.copy')}
            </p>
          </div>
        </div>
        ...
        <div
          ...
          className="rounded-3xl bg-natural-dark text-white p-6 sm:p-10 lg:p-12 shadow-2xl relative overflow-hidden border border-[#4a3535]"
        >
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
            ...
            <div className="lg:col-span-7 flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-natural-rose animate-ping" />
                  <span className="text-xs uppercase tracking-wider font-bold text-natural-border-warm">
                    {t('about.behindScenes')}
                  </span>
                </div>
                <span className="text-xs text-natural-sand font-mono">
                  {t('about.sceneOf')} {activeSceneIndex + 1} {t('about.of')} {kitchenScenes.length}
                </span>
              </div>
              ...
              <div className="relative aspect-video rounded-2xl overflow-hidden bg-black border border-[#4a3535] shadow-inner group">
                ...
                <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full text-xs font-bold text-white flex items-center gap-1.5 border border-white/10">
                  <Flame className="w-3.5 h-3.5 text-natural-rose" />
                ...
                <button
                  ...
                  aria-label={t('about.fullscreenVideo')}
                >
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <button
                    ...
                    className="w-16 h-16 rounded-full bg-natural-rose hover:bg-[#d89393] text-white flex items-center justify-center shadow-lg transform hover:scale-110 transition-all duration-200 border-2 border-white/30 backdrop-blur-xs"
                    aria-label={isPlaying ? t('about.pause') : t('about.play')}
                  >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-linear-to-t from-black/90 via-black/60 to-transparent flex flex-col gap-2">
                  <div
                  ...
                  >
                    <div
                      className="bg-linear-to-r from-natural-rose to-natural-border-warm h-full rounded-full transition-all duration-150"
                      style={{ width: `${progress}%` }}
                    />
                  </div>
                  <div className="flex items-center justify-between text-xs text-natural-sand">
                    <div className="flex items-center gap-3">
                      <button
                        onClick={() => setIsPlaying(!isPlaying)}
                        className="hover:text-white transition-colors font-bold uppercase tracking-wider text-[10px]"
                      >
                        {isPlaying ? t('about.pause') : t('about.play')}
                      </button>
                      <button
                      ...
                      >
                        ...
                        <span>{isMuted ? t('about.muted') : t('about.audioOn')}</span>
                      </button>
                    </div>
                    <span className="font-mono text-natural-sand">{currentScene.duration}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 flex flex-col justify-between space-y-6">
              <div>
                <div className="flex items-center gap-2 text-natural-rose text-xs font-bold uppercase tracking-wider mb-2">
                  <ChefHat className="w-4 h-4" />
                  <span>{t('about.craftsmanship')}</span>
                </div>
                <h3 className="font-serif text-2xl sm:text-3xl font-bold text-white mb-3">
                  {currentScene.title}
                </h3>
                <p className="text-natural-sand text-sm sm:text-base leading-relaxed mb-6 font-normal">
                ...
                <div className="space-y-2.5 mb-6">
                  {currentScene.keyHighlights.map((highlight, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs sm:text-sm text-stone-200">
                      <CheckCircle2 className="w-4 h-4 text-natural-rose shrink-0" />
                      <span>{highlight}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="pt-4 border-t border-[#4a3535]">
                <div className="text-[10px] uppercase tracking-widest text-natural-sand font-bold mb-3">
                  {t('about.selectPerspective')}
                </div>
                    ...
                    <button
                      ...
                      className={`p-2.5 rounded-2xl text-left transition-all text-xs border ${
                        activeSceneIndex === idx
                          ? 'bg-[#4a3535] border-natural-rose text-white font-bold'
                          : 'bg-[#3a2727]/60 border-[#4a3535] text-natural-sand hover:text-white hover:bg-[#3a2727]'
                      }`}
                    >
                      <div className="font-bold truncate">{idx + 1}. {scene.tag}</div>
                      <div className="text-[10px] text-natural-sand mt-0.5">{scene.duration}</div>
                    </button>
                  ))}
                </div>
              </div>
      ...
      {showVideoModal && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="relative w-full max-w-4xl bg-natural-dark rounded-3xl overflow-hidden border border-[#4a3535] shadow-2xl">
            <button
              onClick={() => setShowVideoModal(false)}
              className="absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-black/70 text-white flex items-center justify-center hover:bg-natural-terracotta transition-colors"
            >
              ✕
            </button>
            <div className="p-6">
              <div className="text-natural-rose text-xs font-bold uppercase tracking-widest mb-1">
                {t('about.modalExperience')}
              </div>
              ...
              <div className="flex items-center justify-between">
                <p className="text-natural-sand text-sm italic">{currentScene.description}</p>
                <button
                  onClick={() => setShowVideoModal(false)}
                  className="bg-natural-terracotta text-white px-6 py-2 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-natural-terracotta-dark shrink-0 ml-4"
                >
                  {t('about.close')}
                </button>
              </div>
